  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="grid md:grid-cols-2 gap-10 lg:gap-16 mb-16">
        ...
        {/* Images */}
        <div className="space-y-4">
          <div className="aspect-square rounded-3xl overflow-hidden bg-pink-50">
        ...
        <div className="space-y-6">
          ...
          <div>
            <div className="flex items-center gap-2 mb-2">
              <span className="badge-pink capitalize">
                {product.category?.replace('-', ' ')}
              </span>
              {product.newArrival && (
                <span className="badge bg-purple-100 text-purple-600">✨ New</span>
              )}
              {product.bestSeller && (
                <span className="badge bg-yellow-100 text-yellow-600">⭐ Best Seller</span>
              )}
            </div>
            <h1 className="font-display text-3xl md:text-4xl font-semibold text-rose-900 leading-tight">
              {product.title}
            </h1>
          </div>
          ...
          <div className="flex items-baseline gap-3">
            <span className="font-display text-4xl font-bold text-rose-700">
          ...
          <div
            className={`flex items-center gap-2 text-sm font-medium ${
              product.stock > 0 ? 'text-green-600' : 'text-red-400'
            }`}
          >
            <div
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {product.colors.map(color => (
                  <span
                    key={color}
                    className="px-3 py-1 bg-pink-50 border border-pink-100 rounded-full text-sm text-gray-600"
                  >
                    {color}
                  </span>
                ))}
          ...
          <motion.button
            ...
            className={`w-full flex items-center justify-center gap-3 py-4 rounded-2xl font-semibold transition-all text-base ${
              inWishlist
                ? 'bg-pink-500 text-white shadow-petal'
                : 'bg-pink-50 text-pink-500 hover:bg-pink-100 border-2 border-pink-200'
            }`}
          >
          ...
          {/* WhatsApp Order CTA — THIS WAS THE BROKEN TAG */}
          <a
            href="https://wa.me/919876543210"
            target="_blank"
            rel="noreferrer"
            className="w-full flex items-center justify-center gap-3 py-4 rounded-2xl font-semibold bg-green-500 text-white hover:bg-green-600 transition-colors"
          >
          ...
          <div className="flex flex-col gap-2 pt-2 border-t border-pink-100">
            ...
            <div className="flex items-center gap-2 text-sm text-gray-500">
              <span>🌸</span> Handcrafted with love and care
            </div>
            <div className="flex items-center gap-2 text-sm text-gray-500">
              <span>💬</span> Questions? Chat with us on WhatsApp
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Reviews Section */}
      <div className="mb-16">
        <h2 className="font-display text-2xl font-semibold text-rose-900 mb-8">
          Customer Reviews
        </h2>
                ...
                <div>
                  ...
                  <div className="flex gap-1">
                    {[1, 2, 3, 4, 5].map(s => (
                      ...
                      >
                        <span className={s <= reviewRating ? 'text-amber-400' : 'text-gray-200'}>
                          ★
                        </span>
                      </button>
                    ))}
